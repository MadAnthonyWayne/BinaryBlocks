...
Layer display_layer_hour; 	//hour cells
...
void draw_cell_column_for_digit(GContext* ctx, unsigned short digit, unsigned short max_rows_to_display, unsigned short cell_column) {
	...
	for (int cell_row_index = 0; cell_row_index < max_rows_to_display; cell_row_index++) {
		draw_cell(ctx, 12 + cell_column*36, 9 + (3 - cell_row_index)*36, 18, 18, (digit >> cell_row_index) & 0x1); 
	}
}
...
void display_layer_update_callback(Layer *me, GContext* ctx) {
 ...
 // unsigned short display_hour = get_display_hour(t.tm_hour);
	...
	//draw hour cells
	// done below  draw_cell_column_for_hour_digit(ctx, display_hour % 12, DefaultMaxRows, HourOffset);
	//draw minute cells
...
//update hour cell
void display_layer_update_callback_hour(Layer *me, GContext* ctx) {
	PblTm t;
...
{
	text_layer_init(&TopDayLayer, GRect(57, 136, 72, 15));
...
{
	text_layer_init(&BottomDayLayer, GRect(0, 152, 144, 16));
...
void handle_init(AppContextRef ctx) {
  ...
  window_set_background_color(&window, GColorBlack);
	...
	layer_init(&display_layer, GRect(60, 0, 84, 135));
	display_layer.update_proc = &display_layer_update_callback;
	layer_add_child(&window.layer, &display_layer);
	
	//initiate hour later
	layer_init(&display_layer_hour, GRect(0, 0, 59, 150));
	display_layer_hour.update_proc = &display_layer_update_callback_hour;
	layer_add_child(&window.layer, &display_layer_hour);
